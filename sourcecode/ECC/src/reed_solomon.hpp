...
namespace CNCS::ecc {
    ...
    class ReedSolomon {
    public:
        ReedSolomon(const uint8_t message_length, const uint8_t ecc_lengths)
            : msg_length(message_length), ecc_length(ecc_lengths) {
            ...
            for (uint8_t i = 0; i < msg_length; i++) {
                ...
                if (coef != 0) {
                    for (uint32_t j = 1; j < gen->length; j++) {
                        msg_out->at(i + j) ^= mul(gen->at(j), coef);
                    }
                }
            }

            // Copying ECC to the output buffer
            memcpy(dst_ptr, msg_out->ptr() + msg_length,
                   ecc_length * sizeof(uint8_t));
        }

        /* @brief Message encoding
         * @param *src - input message buffer      (msg_length size)
         * @param *dst - output buffer             (msg_length + ecc_length size
         * at least) */
         ...
         * @param *msg_out     - output buffer            (msg_length size at
         * least)
        ...
        int DecodeBlock(const void* src, const void* ecc, void* dst,
        ...
        /* @brief Message block decoding
         * @param *src         - encoded message buffer   (msg_length +
         * ecc_length size)
         * @param *msg_out     - output buffer            (msg_length size at
         * least)
         * @param *erase_pos   - known errors positions
         * @param erase_count  - count of known errors
         * @return RESULT_SUCCESS if successful, error code otherwise */
        int Decode(const void* src, void* dst, uint8_t* erase_pos = NULL,
    ...
    private:
        ...
        void CalcSyndromes(const Poly* msg) {
            ...
            for (uint8_t i = 1; i < ecc_length + 1; i++) {
                synd->at(i) = poly_eval(msg, pow(2, i - 1));
            }
        }
        ...
        void FindErrorEvaluator(const Poly* synd, const Poly* errata_loc,
        ...
        void CorrectErrata(const Poly* synd, const Poly* err_pos,
                           const Poly* msg_in) {
            ...
            /* uses T_POLY 1, 2 */
            FindErrorEvaluator(rsynd, errata_loc, re_eval,
                               errata_loc->length - 1);
            ...
            Poly* X =
                &polynoms[ID_TPOLY1]; /* this will store errors positions */
            X->length = 0;
            ...
            for (uint8_t i = 0; i < X->length; i++) {
                Xi_inv = inverse(X->at(i));

                err_loc_prime_temp->length = 0;
                for (uint8_t j = 0; j < X->length; j++) {
                    if (j != i) {
                        err_loc_prime_temp->Append(
                            sub(1, mul(Xi_inv, X->at(j))));
                    }
                }
                ...
                for (uint8_t j = 0; j < err_loc_prime_temp->length; j++) {
                    err_loc_prime =
                        mul(err_loc_prime, err_loc_prime_temp->at(j));
                }

                y = poly_eval(re_eval, Xi_inv);
                y = mul(pow(X->at(i), 1), y);

                E->at(err_pos->at(i)) = div(y, err_loc_prime);
            }

            poly_add(msg_in, E, corrected);
        }

        bool FindErrorLocator(const Poly* synd, Poly* erase_loc = NULL,
                              size_t erase_count = 0) {
            ...
            for (uint8_t i = 0; i < ecc_length - erase_count; i++) {
                ...
                for (uint8_t j = 1; j < err_loc->length; j++) {
                    ...
                    delta ^= mul(err_loc->at(index), synd->at(K - j));
                }
                ...
                if (delta != 0) {
                    if (old_loc->length > err_loc->length) {
                        ...
                        poly_scale(err_loc, old_loc, inverse(delta));
                        err_loc->Copy(temp);
                    }
                    poly_scale(old_loc, temp, delta);
        ...
        bool FindErrors(const Poly* error_loc, size_t msg_in_size) {
            ...
            for (uint8_t i = 0; i < msg_in_size; i++) {
                if (poly_eval(error_loc, pow(2, i)) == 0) {
        ...
        void CalcForneySyndromes(const Poly* synd, const Poly* erasures_pos,
                                 size_t msg_in_size) {
            ...
            for (uint8_t i = 0; i < erasures_pos->length; i++) {
                erase_pos_reversed->Append(msg_in_size - 1 -
                                           erasures_pos->at(i));
            }
            ...
            uint8_t x;
            for (uint8_t i = 0; i < erasures_pos->length; i++) {
                x = pow(2, erase_pos_reversed->at(i));
                for (int8_t j = 0; j < forney_synd->length - 1; j++) {
                    forney_synd->at(j) =
                        mul(forney_synd->at(j), x) ^ forney_synd->at(j + 1);
                }
            }
        }
    };
