...
namespace CNCS::ecc {
    ...
    const uint8_t exp[255] = {
        0x1,  0x2,  0x4,  0x8,  0x10, 0x20, 0x40, 0x80, 0x1d, 0x3a, 0x74, 0xe8,
        0xcd, 0x87, 0x13, 0x26, 0x4c, 0x98, 0x2d, 0x5a, 0xb4, 0x75, 0xea, 0xc9,
        0x8f, 0x3,  0x6,  0xc,  0x18, 0x30, 0x60, 0xc0, 0x9d, 0x27, 0x4e, 0x9c,
        0x25, 0x4a, 0x94, 0x35, 0x6a, 0xd4, 0xb5, 0x77, 0xee, 0xc1, 0x9f, 0x23,
        0x46, 0x8c, 0x5,  0xa,  0x14, 0x28, 0x50, 0xa0, 0x5d, 0xba, 0x69, 0xd2,
        0xb9, 0x6f, 0xde, 0xa1, 0x5f, 0xbe, 0x61, 0xc2, 0x99, 0x2f, 0x5e, 0xbc,
        0x65, 0xca, 0x89, 0xf,  0x1e, 0x3c, 0x78, 0xf0, 0xfd, 0xe7, 0xd3, 0xbb,
        0x6b, 0xd6, 0xb1, 0x7f, 0xfe, 0xe1, 0xdf, 0xa3, 0x5b, 0xb6, 0x71, 0xe2,
        0xd9, 0xaf, 0x43, 0x86, 0x11, 0x22, 0x44, 0x88, 0xd,  0x1a, 0x34, 0x68,
        ...
        0x82, 0x19, 0x32, 0x64, 0xc8, 0x8d, 0x7,  0xe,  0x1c, 0x38, 0x70, 0xe0,
        0xdd, 0xa7, 0x53, 0xa6, 0x51, 0xa2, 0x59, 0xb2, 0x79, 0xf2, 0xf9, 0xef,
        0xc3, 0x9b, 0x2b, 0x56, 0xac, 0x45, 0x8a, 0x9,  0x12, 0x24, 0x48, 0x90,
        0x3d, 0x7a, 0xf4, 0xf5, 0xf7, 0xf3, 0xfb, 0xeb, 0xcb, 0x8b, 0xb,  0x16,
        0x2c, 0x58, 0xb0, 0x7d, 0xfa, 0xe9, 0xcf, 0x83, 0x1b, 0x36, 0x6c, 0xd8,
        0xad, 0x47, 0x8e};

    const uint8_t log[256] = {
        0x0,  0x0,  0x1,  0x19, 0x2,  0x32, 0x1a, 0xc6, 0x3,  0xdf, 0x33, 0xee,
        0x1b, 0x68, 0xc7, 0x4b, 0x4,  0x64, 0xe0, 0xe,  0x34, 0x8d, 0xef, 0x81,
        0x1c, 0xc1, 0x69, 0xf8, 0xc8, 0x8,  0x4c, 0x71, 0x5,  0x8a, 0x65, 0x2f,
        0xe1, 0x24, 0xf,  0x21, 0x35, 0x93, 0x8e, 0xda, 0xf0, 0x12, 0x82, 0x45,
        0x1d, 0xb5, 0xc2, 0x7d, 0x6a, 0x27, 0xf9, 0xb9, 0xc9, 0x9a, 0x9,  0x78,
        0x4d, 0xe4, 0x72, 0xa6, 0x6,  0xbf, 0x8b, 0x62, 0x66, 0xdd, 0x30, 0xfd,
        ...
        0xfa, 0x85, 0xba, 0x3d, 0xca, 0x5e, 0x9b, 0x9f, 0xa,  0x15, 0x79, 0x2b,
        0x4e, 0xd4, 0xe5, 0xac, 0x73, 0xf3, 0xa7, 0x57, 0x7,  0x70, 0xc0, 0xf7,
        0x8c, 0x80, 0x63, 0xd,  0x67, 0x4a, 0xde, 0xed, 0x31, 0xc5, 0xfe, 0x18,
        ...
        0x7f, 0xc,  0x6f, 0xf6, 0x6c, 0xa1, 0x3b, 0x52, 0x29, 0x9d, 0x55, 0xaa,
        0xfb, 0x60, 0x86, 0xb1, 0xbb, 0xcc, 0x3e, 0x5a, 0xcb, 0x59, 0x5f, 0xb0,
        0x9c, 0xa9, 0xa0, 0x51, 0xb,  0xf5, 0x16, 0xeb, 0x7a, 0x75, 0x2c, 0xd7,
     ...
     * @return x + y */
    inline uint8_t add(uint8_t x, uint8_t y) { return x ^ y; }
     ...
     * @param y - right operand
     * @return x - y */
    inline uint8_t sub(uint8_t x, uint8_t y) { return x ^ y; }
     ...
     * @param x     - scalar */
    inline void poly_scale(const Poly* p, Poly* newp, uint16_t x) {
     ...
     * @param &newp - destination polynomial */
    inline void poly_add(const Poly* p, const Poly* q, Poly* newp) {
     ...
     * @param &q    - left operand polynomial
     * @param &newp - destination polynomial */
    inline void poly_mul(const Poly* p, const Poly* q, Poly* newp) {
        newp->length = p->length + q->length - 1;
        memset(newp->ptr(), 0, newp->length * sizeof(uint8_t));
        /* Compute the polynomial multiplication (just like the outer
         * product of two vectors, we multiply each coefficients of p with
         * all coefficients of q) */
        for (uint8_t j = 0; j < q->length; j++) {
            for (uint8_t i = 0; i < p->length; i++) {
                newp->at(i + j) ^=
                    mul(p->at(i), q->at(j)); /* == r[i + j] = gf_add(r[i+j],
                                                gf_mul(p[i], q[j])) */
            }
        }
    }
     ...
     * @param &q    - left operand polynomial
     * @param &newp - destination polynomial */
    inline void poly_div(const Poly* p, const Poly* q, Poly* newp) {
        if (p->ptr() != newp->ptr()) {
        ...
        memmove(newp->ptr(), newp->ptr() + sep,
                (newp->length - sep) * sizeof(uint8_t));
    ...
    inline int8_t poly_eval(const Poly* p, uint16_t x) {
        uint8_t y = p->at(0);
